  ...
  useEffect(() => {
    ...
    const capture = async () => {
      try {
        ...
        const transactionId = data.transactionId || token;
        if (window.opener) {
          window.opener.postMessage(
            { type: "PAYPAL_SUCCESS", orderId, transactionId },
            window.location.origin
          );
          window.close();
        } else {
          window.location.href = `/order-success?orderId=${encodeURIComponent(orderId)}&transactionId=${encodeURIComponent(transactionId)}`;
        }
      } catch (err: any) {
        setStatus("error");
