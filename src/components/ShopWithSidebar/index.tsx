...
import { useGetShopProductsQuery } from "@/redux/services/shopApi";
import {
  mapShopProductToProduct,
...
const ShopWithSidebar = () => {
  ...
  const products: Product[] = useMemo(() => {
    if (!data) return [];
    const raw = getProductsFromResponse(data);
    return raw.map(mapShopProductToProduct);
  }, [data]);

  const total = useMemo(() => getTotalFromResponse(data ?? {}), [data]);
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            ...
            <div className="xl:max-w-[870px] w-full">
                    ...
                    <p>
                      {isLoading ? (
                        ...
                        <>
                          ...
                          of <span className="text-dark">{total}</span> Products
                        </>
                      )}
                    </p>
                  </div>
              ...
              {!isError && isLoading && (
                <p className="py-8 text-center text-dark-2">Loading products...</p>
              )}
              {!isError && !isLoading && products.length === 0 && (
                <p className="py-8 text-center text-dark-2">No products found.</p>
              )}
              ...
              {/* <!-- Products Pagination Start (concept: 1 ... window ... last) --> */}
              {totalPages > 1 && (() => {
                const delta = 5;
                ...
                return (
                  <div className="flex justify-center mt-15">
                    <div className=" shadow-1 rounded-md p-2">
                      <ul className="flex items-center">
                        <li>
                          <button
                            ...
                            className="flex items-center justify-center w-8 h-9 ease-out duration-200 rounded-[3px] hover:text-white hover:bg-blue disabled:text-gray-4 disabled:pointer-events-none"
                          >
                            ...
                            <li key={item}>
                              <button
                                ...
                                className={`flex py-1.5 px-3.5 duration-200 rounded-[3px] hover:text-white hover:bg-blue ${
                                  page === item ? "bg-blue text-white" : "text-dark"
                                }`}
                              >
                                {item}
                              </button>
